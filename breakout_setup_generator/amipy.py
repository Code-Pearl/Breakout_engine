...
try:  # optional: only needed for MongoDB backends
    from pymongo import MongoClient
except ImportError:
    MongoClient = None

try:  # optional: only used by the ffn report helpers
...
def mongo_grab(symbol, dbname, startdate, enddate, interval='60min', resample=False):
    ...
    if MongoClient is None:
        raise ImportError('pymongo is not installed; use csv_grab() instead')
    client = MongoClient()
    dbase = client[dbname]
...
def analyze_portfolio(portfolio, rfr, plot=True):
    ...
    print('\n')
    print('Starting Portfolio Equity: ' + str(portfolio['Total'].iloc[0]))
    print('Final Portfolio Equity: ' + str(portfolio['Total'].iloc[-1]))
...
def analyze_portfolio_ffn(portfolio, rfr):
    ...
    if PerformanceStats is None:
        print('ffn is not installed; skipping ffn report.')
        return
    portfolio = portfolio.ffill().bfill()
    portfolio['Total'] = portfolio.sum(axis=1)
...
class Amipy(object):
    ...
    def run(self, buy, short, sell, cover, buyprice, shortprice, sellprice, coverprice):
        ...
        for i, item in enumerate(zip(buy.values, short.values, sell.values, cover.values,
            ...
            if item[1] > 0: # *** active short *** #
                ...
                mytrades.append({'index': short.index[i], 'direction': 'short',
                                 'lotsize': -lot_size, 'price': shortprice.iloc[i],
                ...
                for cnt, col1, col2, col3 in zip(count(), cover.values[i+1:],
                    ...
                    if (col1 == item[1]) | (col3 > 0):
                        ...
                        mytrades.append({'index': cover.index[cnt+i+1], 'direction': 'cover',
                                         'lotsize': lot_size, 'price': coverprice.iloc[cnt+i+1],
                ...
                mytrades.append({'index': buy.index[i], 'direction': 'buy',
                                 'lotsize': lot_size, 'price': buyprice.iloc[i],
                                 'value': 0, 'equity': myequity[i], 'ticks': 0,
                                 'umargin': umargin})
                ...
                for cnt, col1, col2, col3 in zip(count(), sell.values[i+1:],
                    ...
                    if (col1 > 0) | (col3 > 0):
                        ...
                        mytrades.append({'index': sell.index[cnt+i+1], 'direction': 'sell',
                                         'lotsize': -lot_size, 'price': sellprice.iloc[cnt+i+1],
                                         'value': trd_val, 'equity': value, 'ticks': trd_ticks,
                                         'umargin': 0})
        ...
        if len(mytrades) == 0:
            mytrades = pd.DataFrame(columns=['symbol', 'direction', 'lotsize', 'price',
                                              'value', 'equity', 'ticks', 'umargin'])
            mytrades.index.name = 'index'
        else:
    ...
    def analyze_results_silent(self, rfr):
        ''' analyze trades '''
        trades = self.trades
        total_ret = trades.equity.iloc[-1] / trades.equity.iloc[0] - 1
        ...
        equity = pd.DataFrame(temp, index=idx).ffill().bfill()
    ...
    def analyze_results(self, rfr, plot=True):
        ...
        print(self.symbol)
        print('Starting Equity: ' + str(trades['equity'].iloc[0]))
        print('Final Equity: ' + str(trades['equity'].iloc[-1]))
        print('Total return: {:.2%} '.format(float(total_ret)))
    ...
    def plot_trades(self, startdate, enddate):
        ...
        if tlen > 0:
            ...
            temp = pd.DataFrame(self.ohlc.open.loc[subset])
            ...
            self.ohlc.close.loc[subset].plot(ax=ax1, color='black', label='price')
            plt.show()
            plt.close()
        else:
    ...
    def annual_gains(self, start, end):
        ...
        if self.trades.lotsize.abs().mean() == 1:
            for i in range(start, end+1, 1):
                gain = (equity.loc[str(i)].equity.iloc[-1] - \
                                    equity.loc[str(i)].equity.iloc[0]) / equity.equity.iloc[0]
                gains.append(gain*100)
                years.append(i)
        else:
            for i in range(start, end+1, 1):
                gain = (equity.loc[str(i)].equity.iloc[-1] - \
                                    equity.loc[str(i)].equity.iloc[0])/equity.loc[str(i)].equity.iloc[0]
                gains.append(gain*100)
                years.append(i)
    ...
    def analyze_results_ffn(self, rfr):
        ...
        idx = self.ohlc.close.resample('1D').last().index
        equity = pd.DataFrame(data, index=idx).ffill().bfill()
